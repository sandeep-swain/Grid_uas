...
waypoints_dict = {0:{"waypoint1":(138,133),"waypoint2":(113,97),"waypoint3":(143,257),"waypoint4":(535, 270),"waypoint5":(540,130)},1:{"waypoint1":(57,182),"waypoint2":(60,28),"waypoint3":(74,320),"waypoint4":(631,324),"waypoint5":(545,195)}}
...
base_pwms = ((10,30),(10,30),(50,70),(30,50))
pwm_selector = 1
tracking = False
...
while (True):
    ...
    '''if(index<=1):
        ...
        detected1, center_det1, orientation1, bbox_det1 = aruco.track(frame2,aruco_ids[index])'''
    ...
    if detected1 and start_check1  and cam == 2:
        if index <= 1:
            ...
            initial_orientation1 = initial_orientation
    ...
    #############DECIDING ERROR AND PWM BASED ON DESTINATION AND CURRENT COORDINATES###########################
        ...
        if(abs(error) > 20 or distance_to_target<=20):
            pwm_selector = 0
        else:
            pwm_selector = 1
        ###################################################
        ...
        if "error_past" in locals():
            ...
            if (distance_to_target <= 50):
                pwm = PID_Control(error, de, index, base_pwms[index][pwm_selector])
            else:
                pwm  = PID_Control(error, de, index, base_pwms[index][pwm_selector])
        else:
            if (distance_to_target < 30):
                pwm = PID_Control(error, 0, index, 10)
            else:
